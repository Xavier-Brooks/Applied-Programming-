...
int main(int argc, char *argv[])
{
  ...
  gsl_vector *x_ls; /* least squares solution   */
   ...
   //contains the short command line parameters list
   ...
   ------------------------------------------------------------------------*/
   if ( (order < 1) || (inName == NULL) || (order == 0)){
...
void Norm_FindPoint(int nr, int nc, const DArray *points, gsl_vector *x_ls, int verbose) {
  ...
  //first column of A matrix is 1's
  for(i = 0; i < nr; i++){
	gsl_matrix_set(A, i, 0, 1.0);
  }

  //initialize A with data in "points"
  for(i = 0; i < nr; i++){
	double x = points->Payload[i].x;

	//generate powers across columns
	for(j = 1; j < nc; j++){
  ...
  //debugging prints if verbose set
  if(verbose){
