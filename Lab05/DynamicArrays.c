...
unsigned int PushToDArray(DArray *DArrayHead, Data *PayloadPtr)
{
   ...
   ************************************************************************/
   if(DArrayHead->EntriesUsed == DArrayHead->Capacity){
	...
	if(DArrayHead->Payload == NULL){
		printf("failed at reallocating array");
		exit(REALLOC_ERROR);
	}
   }
...
Data *SearchDArray(DArray *DArrayHead, char *String) {
   ...
   /*-----  Don't implement this until instructed in Lab 5 ----*/
	Data *data_p;
	for(int i = 0; i < DArrayHead->Capacity; i++){
		data_p = &(DArrayHead->Payload[i]);
		if(!strcmp(String, (DArrayHead->Payload[i]).String)){
			return data_p;
		}
	}
  	return NULL;
