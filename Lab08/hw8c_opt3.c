...
int main(int argc, char *argv[]){
   ...
   DECLARE_REPEAT_VAR(dataRep)
   DECLARE_REPEAT_VAR(calcRep)
   ...
   if (1 != argc)
    ...
    // Note: Opening a file that is already open has unpredictabl
    ...
    BEGIN_REPEAT_TIMING(REPEAT_NUM,dataRep)
    ...
    BEGIN_REPEAT_TIMING(REPEAT_NUM2,calcRep)
   ...
   PRINT_RTIMER(calcTimer, REPEAT_NUM2)
   PRINT_TIMER(dataTimer)
   PRINT_RTIMER(dataTimer,REPEAT_NUM)
   ...
   for (lcv=0; lcv < DataSet->NextElement; lcv+=2)
    {
      ...
      T_XX += DataSet->Data_X[lcv + 1] * DataSet->Data_X[lcv + 1];
      T_XY += DataSet->Data_X[lcv + 1] * DataSet->Data_Y[lcv + 1];
      T_X  += DataSet->Data_X[lcv + 1];
      T_Y  += DataSet->Data_Y[lcv + 1];
    } /* for() */

   /* Compute the parameters of the line Y = A*X + B */
   (*A) = (((DataSet->NextElement * (S_XY + T_XY)) - ((S_X + T_X) * (S_Y + T_Y))) / ((DataSet->NextElement * (S_XX + T_XX)) - ((S_X + T_X) * (S_X + T_X))));
   (*B) = ((((S_XX + T_XX) * (S_Y + T_Y)) - ((S_XY + T_XY) * (S_X + T_X))) / ((DataSet->NextElement * (S_XX + T_XX)) - ((S_X + T_X) * (S_X + T_X))));
